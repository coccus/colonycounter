...
void JpyrSegmentation(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]);
void JadaptiveThreshold(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]);
...
void mexFunction(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
	...
	else if (!strncmp(funName,"pyrSegmentation",5))
		JpyrSegmentation(n_out, plhs, n_in, prhs);
    
    else if (!strncmp(funName,"adaptiveThreshold",5))
        JadaptiveThreshold(n_out, plhs, n_in, prhs);    
...
void JpyrSegmentation(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
	int nx, ny, nx2, ny2, nz2, nBands, nBytes, img_depth, inplace = FALSE;
	double *ptr_d, threshold1, threshold2;
    int level;
    struct CV_CTRL *Ctrl;
	void *New_Cv_Ctrl (), Free_Cv_Ctrl (struct CV_CTRL *C);
	IplImage *src, *dst;
	...
	/* ---- Check for errors in user's call to function. ----------------------------- */
	if (n_in == 1) { /*add..();*/	return; }
	if (n_in < 5)
		mexErrMsgTxt("PYRSEGMENTATION ERROR: not enough input arguments!");
    
	ny = mxGetM(prhs[1]);    nx = getNK(prhs[1],1);    nBands = getNK(prhs[1],2);

	ptr_d = (double *)mxGetData(prhs[2]);    level = (int)ptr_d[0];
	ptr_d = (double *)mxGetData(prhs[3]);    threshold1  = ptr_d[0];
    ptr_d = (double *)mxGetData(prhs[4]);    threshold2 = ptr_d[0];
    
    if ((level < 1) || (level > 4))
		mexErrMsgTxt("PYRSEGMENTATION ERROR: level must be between 1 and 4!");
    ...
    levelmask = ~-(1<<level);
    if (src->width & levelmask)
		mexErrMsgTxt("PYRSEGMENTATION ERROR: image width mismatch!");
    if (src->height & levelmask)
		mexErrMsgTxt("PYRSEGMENTATION ERROR: image height mismatch!");
...
void JadaptiveThreshold(int n_out, mxArray *plhs[], int n_in, const mxArray *prhs[]) {
	int nx, ny, nx2, ny2, nz2, nBands, nBytes, img_depth, inplace = FALSE;
	double *ptr_d, max_value, param1;
    int adaptive_method, threshold_type, block_size;
    struct CV_CTRL *Ctrl;
	void *New_Cv_Ctrl (), Free_Cv_Ctrl (struct CV_CTRL *C);
	IplImage *src, *dst;
	...
	if (n_in == 1) { /*add..();*/	return; }
	if (n_in < 7)
		mexErrMsgTxt("ADAPTIVE THRESHOLD ERROR: not enough input arguments!");
    
	ny = mxGetM(prhs[1]);    nx = getNK(prhs[1],1);    nBands = getNK(prhs[1],2);

	ptr_d = (double *)mxGetData(prhs[2]);    max_value = ptr_d[0];
	ptr_d = (double *)mxGetData(prhs[3]);    adaptive_method  = (int)ptr_d[0];
    ptr_d = (double *)mxGetData(prhs[4]);    threshold_type = (int)ptr_d[0];
    ptr_d = (double *)mxGetData(prhs[5]);    block_size = (int)ptr_d[0];
	ptr_d = (double *)mxGetData(prhs[6]);    param1 = ptr_d[0];
